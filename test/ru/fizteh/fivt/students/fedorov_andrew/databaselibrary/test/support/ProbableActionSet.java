...
/**
 * Class for convenient probable action set forming and picking random actions
 * from this set.
 * 
 * @author phoenix
 * 
...
public class ProbableActionSet<Action> {
    ...
    public static <E extends Enum<E>> ProbableActionSet<E> makeDistributedSet(
	    Class<E> clazz, int... probabilities) {
	E[] enums = clazz.getEnumConstants();
	if (probabilities.length != enums.length) {
	    throw new IllegalArgumentException(
		    "Probabilities count does not match actions count");
	}
    ...
    public static <E extends Enum<E>> ProbableActionSet<E> makeEquallyDistributedSet(
	    Class<E> clazz) {
	E[] enums = clazz.getEnumConstants();
	ProbableActionSet<E> set = new ProbableActionSet<>();
	for (E en : enums) {
