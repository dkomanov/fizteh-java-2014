...
import ru.fizteh.fivt.students.sautin1.telnet.storeable.TableRow;
...
public class StoreableTest {
    ...
    public void testToString() throws Exception {
        ...
        for (int columnIndex = 0; columnIndex < values.length; ++columnIndex) {
            row.setColumnAt(columnIndex, values[columnIndex]);
        }
        List<Object> valueList = Arrays.asList(values);
        String columns = valueList.stream().map(Object::toString).collect(joining(","));
        StringBuilder correctAnswer = new StringBuilder();
        correctAnswer.append(row.getClass().getSimpleName());
        correctAnswer.append("[");
        correctAnswer.append(columns);
        correctAnswer.append("]");
        assertEquals(correctAnswer.toString(), row.toString());
    }

    @Test
    public void testToStringNull() throws Exception {
        Object[] values = new Object[] {null, null, null, null, null, null, null};
        for (int columnIndex = 0; columnIndex < values.length; ++columnIndex) {
            row.setColumnAt(columnIndex, values[columnIndex]);
        }
        StringBuilder correctAnswer = new StringBuilder();
