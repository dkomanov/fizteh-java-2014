...
class Parser {
	...
	void cp(final File path, final String[] comand) {
		...
		if (comand[1].equals("-r")) {
			...
			if (!obj.exists()) {
				System.out
						.println("there is no file at the root directory of the");
				return;
			}
			if (!object.exists()) {
				System.out
	...
	void mv(final File path, final String[] comand) {
		...
		if (arg1path && arg2path) {
			if ((arg1.isDirectory() && arg2.isDirectory())
					|| (!arg1.isDirectory() && arg2.isDirectory())) {
...
class MethodsDirectory {
	...
	void cp(final String dir, final String place, final boolean flag,
			final File current) {
