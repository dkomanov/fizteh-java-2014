...
public abstract class ShellParser {
	public static boolean parse(String cmd) {
		cmd = cmd.trim();
		String cmdArgs = "";
		String cmdWithoutArgs = cmd;
		int cmdWithoutArgsEnds = cmd.indexOf(" ");
		if (cmdWithoutArgsEnds != -1) {
			cmdArgs = cmd.substring(cmdWithoutArgsEnds + 1);
			cmdWithoutArgs = cmd.substring(0, cmdWithoutArgsEnds);
		}
	...
	private static boolean pwd() {
		try {
			...
			System.out.println("pwd: cannot read current working directory: access denied");
		}
		return false;
	}
	...
	private static boolean ls() {
		try {
			...
			System.out.println("ls: cannot get the list of files: access denied");
		}
		return false;
	}
	...
	private static boolean cd(String cmdArgs) {
		...
		try {
			...
			if (newWorkingDir.exists()) {
				...
				System.out.println("cd: '" + newWorkingDir.getPath() + "': No such file or directory");
		}
		catch (InvalidPathException e) {
			System.out.println("cd: cannot change directory: illegal character in path");
		}
		catch (SecurityException e) {
	...
	private static boolean mkdir(String cmdArgs) {
		...
		try {	
			File makedDir = FileSystems.getDefault().getPath(System.getProperty("user.dir"), cmdArgs).toFile();
			if (!makedDir.mkdir()) {
				System.out.println("mkdir: cannot create directory '" + cmdArgs + "': File exists");
				return false;
			}
			return true;
		}
		catch (InvalidPathException e) {
			System.out.println("mkdir: cannot create directory '" + cmdArgs + "': illegal character in name");
		}
		catch (SecurityException e) {
			System.out.println("mkdir: cannot create directory '" + cmdArgs + "': access denied");
		}
		return false;
	}
		...
		try {
			...
			if (cattedFile.exists()) {
				if (cattedFile.isFile()) {
					...
					System.out.println("cat: " + cmdArgs + ": This is a directory");
			}
			else
				System.out.println("cat: " + cmdArgs + ": No such file or directory");
		}
		catch(SecurityException e) {
			System.out.println("cat: cannot open file '" + cmdArgs + "': access denied");
		}
		catch (InvalidPathException e) {
			System.out.println("cat: cannot open file '" + cmdArgs + "': illegal character in name");
		}
		return false;
	}
}
