				...
				while (true) {
					...
					for (String str : commands) {
						...
						for (int i = 0; i < currentCmd.length; i++) {
							if (Character.isWhitespace(currentCmd[i]) && !quoteOpened) {
								if (builder.length() != 0) {
						...
						if (quoteOpened) {
							System.out.println("Error parsing statement: odd number of quotes");
						}
						if (builder.length() != 0) {
							cmdArgs.add(builder.toString());
							builder.setLength(0);
						}
						StringParser.parse(cmdArgs.toArray(new String[cmdArgs.size()]), false);
						cmdArgs.clear();
					}
				}
		}
