...
public class Cp {
    public static void run(String[] args) throws Exception {
        ...
        if (args.length <= 2 || (args.length <= 3 && args[1].equals("-r"))) {
            ...
            if (args.length > 4 || (args.length > 3 && args[1].equals("-r"))) {
                throw new Exception("cp : wrong number of arguments");
            }
        }
        try {
            ...
            if (!destFile.isAbsolute()) {
                destFile = Paths
                        .get(System.getProperty("user.dir"),
                                args[afterKeyIndex + 1]).normalize().toFile();
            }
            ...
            if (file.isFile()) {
                ...
                if (destFile.isDirectory()) {
                    destFile = Paths.get(destFile.getPath(), file.getName())
                            .toFile();
                }
                if (destFile.toPath().toString()
                        .equals(file.toPath().toString())) {
                ...
                String compare = destFile.toPath().relativize(file.toPath())
                        .toString();
                if (compare.equals("") || compare.matches("[\\/\\.]+")) {
                    throw new Exception(
                            "cp : cannot copy directory into itself");
                }
                if (!destFile.getParentFile().exists()) {
                    throw new Exception("cp : No such file or directory");
                }
    ...
    public static boolean cpRec(File copied, File destination) {
        if (copied.isDirectory()) {
            ...
            for (File f : copied.listFiles()) {
                if (!cpRec(f,
                        Paths.get(destination.getAbsolutePath(), f.getName())
                                .toFile())) {
                    return false;
                }
            }
