...
public class DatabaseTableOperations implements DatabaseShellOperationsInterface<Table, String, Storeable> {
    ...
    @Override
    public DatabaseTable useTable(String name) {
        DatabaseTable tempTable = (DatabaseTable) provider.getTable(name);
        if (tempTable != null) {
            table = tempTable;
        }
        return tempTable;
    ...
    public Table createTable(String parameters) {
        TableInfo info;
        info = StorableUsage.parseCreateCommand(parameters);
        try {
    ...
    public int exit() {
        try {
            ((DatabaseTableProvider) provider).exit();
        } catch (IOException e) {
            return -1;
        }
        return 0;
    ...
    public void showTables() {
        ((DatabaseTableProvider) provider).showTables();
    }

    @Override
    public List<String> list() {
        return ((DatabaseTable) table).list();
    }

    public void setProvider(TableProvider provider) {
        this.provider = provider;
    }
}
