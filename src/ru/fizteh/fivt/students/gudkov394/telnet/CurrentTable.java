...
public class CurrentTable implements Table, AutoCloseable {
    ...
    public CurrentTable(Path path, ReentrantReadWriteLock thisLock) throws IOException {
        removed.set(new HashSet<>());
//        System.out.println("const = " + removed.get());
        put.set(new HashMap<>());
        ...
        loadData();
//        System.out.println("const end = " + removed.get());
    }

    public CurrentTable(Path path, List<Class<?>> columnTypes, ReentrantReadWriteLock thisLock) throws IOException {
        removed.set(new HashSet<>());
        put.set(new HashMap<>());
        changed.set(new HashMap<>());
        dbPath = path;
    ...
    public int commit() throws IOException, IllegalStateException {
        ...
        try {
            ...
            removed.get().forEach(table::remove);
            ...
            changed.get().clear();
            removed.get().clear();
//            System.out.println("after clear = " + removed.get());
...
//        System.out.println("get = " + removed.get());
...
//        System.out.println("put = " + removed.get());
        if (removed.get().contains(key)) {
            valueBefore = null;
...
//        System.out.println("changed = " + changed);
    ...
    int changesNumber() {
        return changed.get().size() + put.get().size() + removed.get().size();
    }
    ...
    public void check() {
        System.out.println("removed.get = " + removed.get());
        System.out.println("changed.get = " + changed.get());
        System.out.println("put.get = " + put.get());
    }
}
