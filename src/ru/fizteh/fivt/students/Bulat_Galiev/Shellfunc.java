...
public class Shellfunc {
	...
	File dir = new File(System.getProperty("user.dir")); // workspace
							     // directory
    ...
    public static void cp(String arg1, String arg2, String arg3)
	    throws IOException {
	if (arg1.equals("-r")) {
    ...
    public static void reccopy(File fsrc, File fdst) throws IOException {
	try {
	    if (fsrc.isDirectory()) {
    ...
    public static void cat(String fname) throws IOException {
	if (fname == null) {
	    System.out.println("wrong arguments");
	    return;
	}
    ...
    public static void rm(String arg1, String arg2) throws IOException {
	if (arg1.equals("-r")) {
    ...
    public static boolean recremove(File dfile) throws IOException {
    ...
    public static boolean remove(File dfile) throws IOException {
	...
	if (newDir.exists()) {
	    ...
	    if (!newDir.isAbsolute()) { // C:\\ is absolute
		origPath = newDir.getPath();
		newDir = new File(newDir.getAbsolutePath());
	    }
	    if (!newDir.isDirectory()) {

		System.out.println((origPath == null ? newDir.getPath()
			: origPath) + (" is not a Directory"));
		return;
	    }
	    try {
		System.setProperty("user.dir", newDir.getCanonicalPath()); // without
									   // ..
	    } catch (IOException e) {
		e.printStackTrace();
	    }
