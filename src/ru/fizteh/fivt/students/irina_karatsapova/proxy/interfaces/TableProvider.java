...
/**
 * Управляющий класс для работы с {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Table таблицами}
 *
...
public interface TableProvider extends AutoCloseable {
    ...
    /**
     * Преобразовывает строку в объект {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}, соответствующий структуре таблицы.
     *
     * @param table Таблица, которой должен принадлежать {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}.
     * @param value Строка, из которой нужно прочитать {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}.
     * @return Прочитанный {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}.
     *
    ...
    /**
     * Преобразовывает объект {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable} в строку.
     *
     * @param table Таблица, которой должен принадлежать {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}.
     * @param value {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}, который нужно записать.
     * @return Строка с записанным значением.
     *
     * @throws ru.fizteh.fivt.students.irina_karatsapova.proxy.exceptions.ColumnFormatException При несоответствии типа в {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable} и типа колонки в таблице.
     */
    String serialize(Table table, Storeable value) throws ColumnFormatException;

    /**
     * Создает новый пустой {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable} для указанной таблицы.
     *
     * @param table Таблица, которой должен принадлежать {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}.
     * @return Пустой {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}, нацеленный на использование с этой таблицей.
     */
    Storeable createFor(Table table);

    /**
     * Создает новый {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable} для указанной таблицы, подставляя туда переданные значения.
     *
     * @param table Таблица, которой должен принадлежать {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}.
     * @param values Список значений, которыми нужно проинициализировать поля Storeable.
     * @return {@link ru.fizteh.fivt.students.irina_karatsapova.proxy.interfaces.Storeable}, проинициализированный переданными значениями.
     * @throws ru.fizteh.fivt.students.irina_karatsapova.proxy.exceptions.ColumnFormatException При несоответствии типа переданного значения и колонки.
