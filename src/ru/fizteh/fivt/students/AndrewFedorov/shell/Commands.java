...
public class Commands {
    ...
    if (!Files.exists(sourcePath)) {
        Utility.handleError(String.format("%s: %s: No such file or directory",
            args[0], srcName));
    }
    ...
    try {
        ...
        Utility.handleError(exc, String.format("%s: cannot %s '%s' to '%s'",
    ...
    @ShellCommand(description = "\tmv <src> <dst>\tmoves/renames file (file tree) under <src> to <dst>."
            + " Actually this command is combination of 'cp' and 'rm'")
    ...
    if (Files.isDirectory(catPath)) {
        Utility.handleError(String.format("cat: %s: Is a directory", catName));
    } else {
        ...
        Utility.handleError(String.format("cat: %s: No such file or directory",
            catName));
        }
    }
    }
    ...
    if (Files.isDirectory(newdir)) {
        ...
        Utility.handleError(String.format("cd: %s: Is not a directory", newdir));
    } else {
        Utility.handleError(String.format("cd: %s: No such file or directory",
            newdir));
    }
    }
}
