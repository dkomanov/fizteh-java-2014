...
public class ParallelTableProvider implements TableProvider {
    ...
    @Override
    public ParallelTable getTable(String name)
    ...
    @Override
    public ParallelTable createTable(String name, List<Class<?>> columnTypes)
        ...
        try {
            if (tables.containsKey(name)) {
                ...
                ParallelTable table = new ParallelTable(this, name, dbDirPath, columnTypes, lock);
    ...
    public Map<String, ParallelTable> getAllTables() {
        lock.readLock().lock();
        try {
            return tables;
        } finally {
