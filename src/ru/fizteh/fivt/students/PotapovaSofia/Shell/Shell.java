...
public class Shell {
	...
	public static void copy(String source, String destination, boolean key)
			throws Exception {
		...
		if (sourceDir.equals(destDir)) {
			throw new Exception("cp: " + source + " and " + destination
					+ " are the same\n");
		}
		if (!sourceDir.exists()) {
	...
	public static void move(String source, String destination) throws Exception {
		...
		File destDir = new File(destPath);
		if ((sourceDir.exists())
				&& (sourceDir.getPath().equals(destDir.getPath()))) {
			return;
		}
		if ((sourceDir.exists()) && (sourceDir.isDirectory())
				&& (!destDir.exists())
				&& (sourceDir.getParent().equals(destDir.getParent()))) {
			if (!sourceDir.renameTo(destDir)) {
				throw new Exception("rm: " + source
						+ "' : No such file or  directory");
			}
			return;
		}
	...
	public static void commandParse(String[] runningCmd, int len)
			throws Exception {
