...
public class Shell extends UserShell {
    ...
    public void makeDirectory(String arg) {
    ...
    public boolean copyErrorCheck(String[] args) {
    ...
    public void copy(String[] args) {
    	...
    	if (args.length == 3 || !currFile.isDirectory()) {
    		try {
    		destFile = Paths.get(destFile.toString(), currFile.toPath().getFileName().toString()).normalize().toFile();
