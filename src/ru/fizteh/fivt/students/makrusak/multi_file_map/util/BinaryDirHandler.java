...
public class BinaryDirHandler {
  ...
  public BinaryDirHandler(File _path) throws IOException {
    path = _path;
    ...
    try {
      ...
      if (parts.length != 2 || !parts[1].equals("dir") || num < 0 || num >= 16) {
        throw new Exception();
      }
    } catch (Exception e) {
    ...
    if (path.listFiles() != null) {
      for (File inFile : path.listFiles()) {
        binaryFileHandlers.put(inFile.getName(), new BinaryFileHandler(inFile));  
      }
    }
  }
  ...
  public String work(String[] tokens) throws IOException {
    if (tokens.length >= 2) {
      ...
      if (binaryFileHandlers.get(fileStr).get_count() == 0) {
