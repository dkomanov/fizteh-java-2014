...
import multifilemap.util.DatabaseHandler;

class MultiFileMap {
    private static void workWithLine(DatabaseHandler handler, String s)
            throws IOException {
        String[] stringPieces = s.split(";");
        for (String piece : stringPieces) {
            List<String> items = new ArrayList<String>(Arrays.asList(piece
                    .split(" ")));
