...
public class Funcshell {
    ...
    public void mkdir(String[] args) {
        if (args.length == 2) {
            ...
            if (!f.exists()) {
                try {
                    if (!f.mkdir()) {
                        System.out.println("Folder isn't create");
                    }
                } catch (Exception e) {
    ...
    public void deletefiledirectories(File f) throws IOException {
        if (f.isDirectory()) {
        ...
        if ((args.length == 4 && args[1].equals("-r")) || (args.length == 3)) {
            ...
            if (Files.exists(f1.toPath()) && Files.exists(f2.toPath())) {
                ...
                if (!Files.exists(f2.toPath())) {
                    System.out.println("cp: cannot copied to '" + args[args.length - 1]
                            + "': No such file or directory ");
                }
            }
        } else { System.out.println("Incorrect arguments"); }
    ...
    public void mv(String[] args) throws IOException {
        if (args.length == 3) {
            ...
            if (f1.exists() && f2.exists()) {
                ...
                if (!f2.exists()) { System.out.println("mv: cannot move '" + args[args.length - 1]
                        + "': No such file or directory ");
                }
            }
        } else { System.out.println("Incorrect arguments"); }
