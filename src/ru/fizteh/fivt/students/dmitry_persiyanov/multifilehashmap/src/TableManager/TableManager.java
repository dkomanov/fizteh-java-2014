...
import DbCommands.CommandsParser;

import java.io.*;
import java.util.*;

public final class TableManager {
    public TableManager(final String TablePath) {
    ...
    public static void main(final String[] args) {
        try {
            dbFile = new RandomAccessFile(TablePath, "rw");
            loadHashMap();
            if (args.length == 0) {
                System.out.print("$ ");
                interactiveMode();
            } else {
                DbCommand[] commands = CommandsParser.parse(args);
                batchMode(commands);
            }
