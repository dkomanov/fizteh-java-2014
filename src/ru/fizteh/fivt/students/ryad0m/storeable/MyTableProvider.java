...
public class MyTableProvider implements TableProvider {
    ...
    public MyTableProvider(Path path) throws IOException {
        ...
        for (File table : tableDirs) {
            if (table.exists() && table.isDirectory()) {
                StructedTable newDirTable =
                        new StructedTable(table.toPath());
    ...
    public Table createTable(String name, List<Class<?>> columnTypes) throws IOException {
        ...
        StructedTable t =
                new StructedTable(
                        location.resolve(name).normalize(), columnTypes);
        tables.put(name, t);
    ...
    public Storeable deserialize(Table table, String value) throws ParseException {
        List<Class<?>> list =
                ((TransactionTable) table).getStructedTable().getColumnTypes();
        return new MyStorable(
                list, XmlSerializer.deserializeString(value, list));
    }

    @Override
    public String serialize(Table table, Storeable value) throws ColumnFormatException {
        ((TransactionTable) table).getStructedTable().checkIntegrity(value);
        return XmlSerializer.serializeObjectList(
                ((MyStorable) value).getValues());
    }
