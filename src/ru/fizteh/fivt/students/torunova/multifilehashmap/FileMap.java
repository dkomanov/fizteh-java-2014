	...
	public FileMap(final String f) throws IncorrectFileException, IOException {
		...
		while (fis.available() > 0) {
			...
			if (fis.read(key) != length) {
				throw new IncorrectFileException("Real key length does not match specified in file length.");
			}
			length = fis.readInt();
			byte[] value = new byte[length];
			if (fis.read(value) != length) {
				throw new IncorrectFileException("Real value length does not match specified in file length.");
			}
