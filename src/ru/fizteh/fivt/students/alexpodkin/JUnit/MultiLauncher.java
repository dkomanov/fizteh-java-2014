...
public class MultiLauncher {
    ...
    private ru.fizteh.fivt.students.alexpodkin.JUnit.Reader reader;
    private Writer writer;
    private boolean exitFlag = false;
    private HashMap<String, Integer> tableNames;
    private PrintStream printStream = System.out;
    private HashMap<String, String>[][] keysPath = null;
    private HashMap<String, String>[][] startMap = null;
    ...
    private boolean writeData() throws IOException {
        ...
        for (int i = 0; i < 15; i++) {
            for (int j = 0; j < 15; j++) {
                String subDir = parentDir + File.separator + currentTable + File.separator + Integer.toString(i)
                        + ".dir";
                writer = new Writer(parentDir
                        + File.separator + currentTable + File.separator
                        + Integer.toString(i) + ".dir" + File.separator + Integer.toString(j) + ".dat");
                writer.writeDataToFile(keysPath[i][j], subDir);
            }
        }
        return true;
    }
    ...
    private boolean sizeCommand(String[] arguments) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                pairCount += keysPath[i][j].size();
            }
        }
        printStream.println(pairCount);
        return true;
    }
    ...
    private int changesCount() {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                for (HashMap.Entry<String, String> point : keysPath[i][j].entrySet()) {
                    if (startMap[i][j].get(point.getKey()) != point.getValue()) {
                        counter++;
                    }
                }
            }
